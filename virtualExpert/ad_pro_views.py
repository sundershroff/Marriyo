...
def create_new_ads(request,id):
    ...
    try:
        age_range_get = request.POST.getlist('age_range')[0]
        age_to_get =  request.POST.getlist('age_to')[0]
    except:
        age_range_get = age_range_get = request.POST('age_range')
        age_to_get =  request.POST('age_to')
    ...
    data = {
        ...
        'languages':json.dumps(list(filter(bool, request.POST.getlist('languages')))),
        'office_country':json.dumps(list(filter(bool,request.POST.getlist('office_country')))),
        'office_state':json.dumps(list(filter(bool,request.POST.getlist('office_state')))),
        'office_district':json.dumps(list(filter(bool,request.POST.getlist('office_district')))),
        'gender':json.dumps(list(filter(bool,request.POST.getlist('gender')))),
        'age_range': age_range_get,
        'age_to': age_to_get,             
        'id_card': full_path,
        'no_views':0,
...
def ad_pro_edit_ads(request,id):
    ...
    data = {
        ...
        'office_district':json.dumps(list(filter(bool,request.POST.getlist('office_district')))),
        'gender':json.dumps(list(filter(bool,request.POST.getlist('gender')))),
        'age_range': json.dumps(list(filter(bool,request.POST.getlist('age_range')))),
        'age_to': json.dumps(list(filter(bool,request.POST.getlist('age_to')))),          
        'id_card': full_path,
        # 'no_views':0,
        'days_required':request.POST['days_required'],
        'times_repeat':request.POST['times_repeat'],
...
def ad_pro_password_reset(request,id):
    try:
        ...
        content = f"""
        PasswordResetform : {f"http://51.20.61.70:3000/ad_pro_password_resett/{id}"}
        """
...
@api_view(['DELETE'])
def ad_pro_delete(request):   
    try:
        print(request.data['email'])
        data_to_delete = ad_pro_serializer.ad_provider.objects.get(email = request.data['email'])
...
@api_view(["POST"])
def ad_pro_notify_status_true(request,id):
    try:
        print(id)
        user=get_object_or_404(models.ad_provider,uid=id)
        print(user)
...
@api_view(["POST"])
def ad_pro_notify_status_false(request,id):
    try:
        print(id)
        user=get_object_or_404(models.ad_provider,uid=id)
        print(user)
...
@api_view(["GET"])
def ad_pro_emera_coin(request,id):
    new=[]
    ...
    print(total_coin)
    return Response(total_coin,status=status.HTTP_200_OK)

@api_view(["GET"])
def ad_pro_amount(request,id):
    ...
    return Response({"commision":totalcommission},status=status.HTTP_200_OK)

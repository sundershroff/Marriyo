...
def hm_upload_account(request,id):
    ...
    if request.method=="POST":
        ...
        if 'degree_cer' in request.FILES:
            ...
            degree_cer=json.dumps(image_list)
        else:
            degree_cer="empty"
        # Aadhaar 
        ...
        if 'expr_certi' in request.FILES:
            ...
            full_path_ex = json.dumps(cert_list)
        else:
            full_path_ex = "empty"
...
def profile_manager_upload_account(request,id):
    try:
        ...
        if "id_card" in request.FILES:
            ...
            full_path_one = userdata.id_card
        print("id1")
        #signed Document
        if "sign_document" in request.FILES:
            print("yes")
            ...
            full_path_two = userdata.sign_document
        print("sign2")
        #verification Image
        if "verification_img" in request.FILES:
    ...
    try:
        ...
        if request.method=="POST":
            ...
            if 'degree_cer' in request.FILES:
                ...
                degree_cer=userdata.degree_cer
            # Aadhaar 
            if 'aadhaar_card' in request.FILES:
                aadhaar_no = request.POST['aadhaar_no']
                aadhaar_card_1= str(request.FILES['aadhaar_card']).replace(" ", "_")
                ...
                full_path_aadhaar = userdata.aadhaar_card
            #  Pan_Card
            if 'pan_card' in request.FILES:
                pan_no = request.POST['pan_no']
                pan_card_1 = str(request.FILES['pan_card']).replace(" ", "_")
            ...
            if 'drive_licence' in request.FILES:
                ...
                drive_licence_no = userdata.drive_licence_no
                drive_licence_date = userdata.drive_licence_date
                licence_state = userdata.licence_state
                full_path_lic = userdata.drive_licence
            ...
            if request.POST['notary_lic_no'] == '':
                ...
                notary_lic_no = request.POST['notary_lic_no']
                notary_issued = str(request.POST['notary_issued'])
                notary_state = request.POST['notary_state']
            ...
            if 'expr_certi' in request.FILES:
                ...
                full_path_ex = userdata.expr_certi
            ...
            if 'work_job_title' not in request.POST or request.POST['work_job_title'] == '':
                work_job_title = userdata.work_job_title
            else:
                work_job_title =json.dumps(request.POST.getlist('work_job_title'))
            print("aaaaaaaaaaaa") 
            if 'work_company_name' not in request.POST or request.POST['work_company_name'] == '':
                work_company_name = userdata.work_company_name
            else:
                work_company_name = json.dumps(request.POST.getlist('work_company_name'))
            if 'work_start_date' not in request.POST or request.POST['work_start_date'] == '':
                work_start_date = userdata.work_start_date
            else:
                work_start_date = json.dumps(request.POST.getlist('work_start_date'))

            if 'starting_salary' not in request.POST or request.POST['starting_salary'] == '':
                starting_salary = userdata.starting_salary
            else:
                starting_salary = json.dumps(request.POST.getlist('starting_salary'))

            if 'work_end_date' not in request.POST or request.POST['work_end_date'] == '':
                work_end_date = userdata.work_end_date
            else:
                work_end_date = json.dumps(request.POST.getlist('work_end_date'))

            if 'final_salary' not in request.POST or request.POST['final_salary'] == '':
                final_salary = userdata.final_salary
            else:
                final_salary = json.dumps(request.POST.getlist('final_salary'))

            if 'reason_leaving' not in request.POST or request.POST['reason_leaving'] == '':
                reason_leaving = userdata.reason_leaving
            else:
                reason_leaving =json.dumps(request.POST.getlist('reason_leaving'))

            if 'work_review_y' not in request.POST or request.POST['work_review_y'] == '':
                work_review_y = userdata.work_review_y
            else:
                work_review_y =json.dumps(request.POST.getlist('work_review_y'))

            if 'skills' not in request.POST or request.POST['skills'] == '':
                skills= userdata.skills
            else:
                skills = json.dumps(request.POST.getlist('skills'))
                
            if 'curent_busines' not in request.POST or request.POST['curent_busines'] == '':
                curent_busines = userdata.curent_busines
            else:
                curent_busines = request.POST['curent_busines']

            if 'past_business' not in request.POST or request.POST['past_business'] == '':
                past_business = userdata.past_business
            else:
                past_business =request.POST['past_business']
            print("previosdataaaaa")
            data = {
                'office_name': request.POST['office_name'],
                'office_country': request.POST['office_country'],
                'office_city': office_city,
            ...
            if basicdetailsserializer.is_valid():
                ...
                instance=userdata1, data=data1, partial=True)
                if hmdetailsserializer.is_valid():
                    userdata.notary_issued = request.POST['notary_issued']
                    userdata.save()
                    hmdetailsserializer.save()
                    print("valid data")
...
# @api_view(['GET'])
# def hm_sendnotification(request, id):
#     channel_layer = get_channel_layer()
#     async_to_sync(channel_layer.group_send)(
#         "notification_broadcast",
#         {
#             'type':'send_notification',
#             'message':json.dumps("sunder")
#         }
#     )
#     return JsonResponse({'success': True})

# from virtualExpert.server import hello
# @api_view(['GET'])
# def hm_notification(request, id):    

#     return JsonResponse({ 'room_name':"broadcast"})

...
def password_reset(request,id):
    try:
        ...
        content = f"""
        PasswordResetform : {f"http://51.20.61.70:3000/password_resett/{id}"}
        """
        yagmail.SMTP(sender, password).send(
            to=email,
            subject=subject,
...
def coin_commision(request,id):
    ...
    return Response({"emera_coin":totalcoin,"commision":totalcommission},status=status.HTTP_200_OK)

...
def ad_dis_password_reset(request,id):
    try:
        ...
        content = f"""
        PasswordResetform : {f"http://51.20.61.70:3000/ad_dis_password_resett/{id}"}
        """
...
@api_view(["GET"])
def ad_dis_emera_coin(request,id):
    new=[]
    # mydata = requests.get(f"http://51.20.61.70:3000//ad_pro_my_data/{id}").json()[0]
    all_data=requests.get("http://51.20.61.70:3000//all_ads_data/").json()
    print(id)
    for i in all_data:
    ...
    print(total_coin)
    return Response({'emera_coin':total_coin},status=status.HTTP_200_OK)

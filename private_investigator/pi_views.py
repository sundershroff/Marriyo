...
def signin(request):
    ...
    if request.method == "POST":
        ...
        if response.status_code == 200:
            my = requests.get(f"http://51.20.61.70:3000/pi_my_data/{uidd}").json()[0]
            ...
            if user_otp is not None:
                ...
                delete_hm = requests.delete("http://51.20.61.70:3000/pi_delete_data/",data=request.POST)
...
def opt_check(request,id):
    ...
    # get = requests.get(f" http://51.20.61.70:3000/otp/{id}").json()
    # print(get['otp'])
    # print(get['uid'])
    ...
    if request.method == "POST":
        ...
        response = requests.post(f"http://51.20.61.70:3000/pi_otp/{id}", data=data)
...
def profile_picture(request,id):
    if request.method == "POST":
        ...
        response = requests.post(f"http://51.20.61.70:3000/pi_profilePicture/{id}",files=request.FILES)
        print(response)
        print(response.status_code)
        print(response.text)
        uidd = (response.text[1:-1])
...
def complete_profile(request,id):
    ...
    if request.method == "POST":
        ...
        print(dictio)
        # response = requests.post(f"http://54.159.186.219:8000/profilepicture/{id}",files=request.FILES)
        response = requests.post(f"http://51.20.61.70:3000/pi_complete_account/{id}",data = dictio,files=request.FILES)
        print(response)
        print(response.status_code)
        print(response.text)
        uidd = (response.text[1:-1])
        ...
        if response.status_code == 200:
            ...
            notify=requests.post("http://51.20.61.70:3000/notification_update/",data=data)
            if notify.status_code == 200:
                status_up=requests.post(f"http://51.20.61.70:3000/pm_notify_status_true/{uid}")
...
def admin_dashboard(request,id):
        ...
        if value != None:
            ...
            return redirect("/pi_signin")
        my = requests.get(f"http://51.20.61.70:3000/pi_my_data/{id}").json()[0]
        pf_users = requests.get("http://51.20.61.70:3000/alluserdata/").json()
        #Notification
        ...
        # print(pf_users)
        my_client = requests.get(f"http://51.20.61.70:3000/pi_my_clients/{id}").json()[id]
        filtered_clients = []
        for x in my_client:
...
def profile(request,id):
        ...
        my_clients = requests.get(f"http://51.20.61.70:3000/pi_my_clients/{id}").json()[id]
        filtered_clients = []
        if len(my_clients) != 0:
            for x in my_clients:
                if "empty" not in x['answer']:
...
def edit_profile(request,id):
        ...
        if request.method == "POST":
            print(request.POST)
            print(request.FILES)
            response = requests.post(f"http://51.20.61.70:3000/pi_edit_account/{id}",data=request.POST,files=request.FILES)
...
def client_list(request,id):
        ...
        my_client = requests.get(f"http://51.20.61.70:3000/pi_my_clients/{id}").json()[id]
        if len(my_client) == 0:
            all_profinder_data_values = requests.get("http://51.20.61.70:3000/alluserdata/").json()
        else:
            all_profinder_data_values=[]
            all_profinder_data = requests.get("http://51.20.61.70:3000/alluserdata/").json()
            ...
            if x['uid'] == client_one:
                ...
                if specific_user['Questin'] is not None:
                   ...
                   print(question)
                    ...
                    question=""
                # question_and_Answer = requests.get(f"http://51.20.61.70:3000/my_question_and_answer/{x['uid']}").json()[x['uid']]
...
def client_feedback(request,id):
        ...
        # print(my_client_one)
        profile_finder_value = requests.get(f"http://51.20.61.70:3000/pi_my_clients/{id}").json()[id]
        ...
        for x in profile_finder_value:
            if my_client_one == x['uid']:
                ...
                # question_and_Answer = requests.get(f"http://51.20.61.70:3000/my_question_and_answer/{x['uid']}").json()[x['uid']]
                # situation_prediction = requests.get(f"http://51.20.61.70:3000/my_question_and_answer/{x['uid']}").json()[x['uid']]
        ...
        if request.method=="POST":
            ...
            response = requests.post(f"http://51.20.61.70:3000/my_question_and_answer/{specific_user['uid']}",data=data)
            print(response)
            print(response.status_code)
            print(response.text)
...
def setting(request,id):
        ...
        if request.method=="POST":
            ...
            if 'pass_reset' in request.POST:
                ...
                response = requests.post(f"http://51.20.61.70:3000/privateinvest_password_reset/{id}",data=request.POST )
            else:
            
                response = requests.post(f"http://51.20.61.70:3000/pi_email_update/{id}", data = request.POST)
                print(response)
                print(response.status_code)
                print(response.text)
   ...
   # else:
    ...
    if request.method=="POST":
        ...
        if request.POST['password'] == request.POST['confirm_password']:
            ...
            response = requests.post(f"http://51.20.61.70:3000/pass_privateInvestigator_update/{id}",data=request.POST )
  ...
  #      print(value)
   ...
   #     return redirect("/pi_signin")
    ...
    if request.method == "POST":
        ...
        print(request.POST)
        response = requests.post("http://51.20.61.70:3000/pi_forget_password/",data=request.POST)
        print(response)
        print(response.status_code)
...
#    value = request.COOKIES.get('private_investigator')
 #   if value != None:
  #      print(value)
   ...
   # else:
    #    return redirect("/pi_signin")
    mydata = requests.get(f"http://51.20.61.70:3000/pi_my_data/{id}").json()[0]
    context = {'invalid':"invalid",
                'key':mydata}
    new=[]
    if request.method == "POST":
        ...
        print(data)
        response = requests.post(f"http://51.20.61.70:3000/pi_forget_password_otp/{id}", data=data)

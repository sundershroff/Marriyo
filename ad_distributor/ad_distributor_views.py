...
def upload_acc(request,id):
    try:
        ...
        if request.method == "POST":
            ...
            print(dictio)
            # response = requests.post(f"http://54.159.186.219:8000/profileidcard/{id}",   files=request.FILES)
            response = requests.post(f"http://127.0.0.1:3000/ad_dis_upload_account/{id}",   data =dictio,files=request.FILES)
            print(response)
            print(response.status_code)
            print(response.text)
            uidd = (response.text[1:-1])
...
def logout_view(request):
    value = request.COOKIES.get('ad_distributor')
    print(value)
    response = redirect("/ad_distributor/signin/")
    # response = HttpResponse("delete cookie")
    response.delete_cookie("ad_distributor")
    return response
...
def edit_account(request,id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            print(response.status_code)
            print(response.text)
            return redirect(f"/ad_distributor/ad_dis_account/{id}")
...
def ad_dis_users(request,id):
    ...
    if request.method== "POST":
        ...
        if "delete" in request.POST:
            response = requests.post(f"http://127.0.0.1:3000/ad_dis_add_user/{idd}",data=request.POST)
            print(response.text)
            print(response.status_code)
            ...
            print(data)
            response = requests.post(f"http://127.0.0.1:3000/ad_dis_add_user/{idd}",data=data)
            # print(response.text)
            # print(response.status_code)
            if response.status_code == 200:
...
def ad_dis_addusers(request,id):
    ...
    if request.method=="POST":
        ...
        if request.POST['password'] == request.POST['confirm_password']:
           ...
           print(response.status_code)
           if response.status_code == 200:
              return redirect(f"http://127.0.0.1:3000/ad_distributor/ad_dis_users/{id}")
           elif response.status_code == 203:
              print("user already exist")
              error = "User Already Exixts"
    context={
  ...
  #      print(value)
   ...
   # else:
    ...
    if request.method == "POST":
        ...
        if response.status_code == 200:
            return redirect(f"/ad_dis_forgetpassword_otpp/{uidd}")
...
def ad_dis_forgetpassword_otp(request,id):
 ...
 #       return redirect("/ad_distributor/signin/")
    ...
    if request.method == "POST":
        ...
        if response.status_code == 200:
            return redirect(f"/ad_dis_forgetpassword_resett/{uidd}")
        else:
            invalid = "Invalid OTP"
            context = {'invalid':invalid}

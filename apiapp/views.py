        ...
        try:
            if extension.validate_email(request.data['email']):
                ...
                try:
                    ...
                    print(data.coin)
                    if data.coin == None or data.coin == "None" or data.coin == "null":
                        data.coin = "1"
                        data.save()
                    else:
                        old_coin = int(data.coin)
                        old_coin += 1
                        data.coin = str(old_coin)
                        data.save()
                except:
                    pass
...
#received_uid
            ...
            if sender_list.is_valid() and received_list.is_valid():
                ...
                data={
                'noter_id':id,
                'not_message':"You Have new request",
                'notify_id': request.POST['received_uid'],
                }
...
def favorites(request, id):
    try:
        if request.method == 'GET':
            ...
            print(id)
            print(request.POST)
            get_image = ProfileFinder.objects.get(uid = id)
            print(get_image.profile_picture)
            notification = models.Notification.objects.create(
                not_id = request.POST['myfavorite_id'],
                notify_id = id,
                not_message = "Some One Liked Your Profile",
                images = str(get_image.profile_picture),
            )
            notification.save()
...
def proads_views_count(request,id):
    if request.method == "POST":
        ...
        response=requests.post(f"http://51.20.61.70:3000/ad_pro_update_coin_value/{ads_id}",data=data2)
...
def disads_views_count(request,id):
    if request.method == "POST":
        ...
        response=requests.post(f"http://51.20.61.70:3000/ad_dis_update_coin_value/{ads_id}",data=data2)
...
@api_view(["GET"])     
def profile_finder_notification(request,id):
    noti = models.Notification.objects.filter(notify_id = id).values()
    return Response(noti,status=status.HTTP_200_OK)

@api_view(["GET"]) 
def related_list(request,id):
    ...
    print(len(lenofid))
    print(lenofid)
    return Response(lenofid,status=status.HTTP_200_OK)

@api_view(['POST'])
def pf_email_update(request,id): 
    if request.method == "POST":        
        print(request.POST)
        userdata1 = serializer.ProfileFinder.objects.get(uid = id)
        userdata1.email = request.POST['email']
        userdata1.save()
        data={
                'noter_id':userdata1.uid,
                'not_message':"Your Email Updated Successfully",
                'notify_id': userdata1.uid,
                }
        notify=requests.post("http://127.0.0.1:3000/notification_update/",data=data)
        if notify.status_code == 200:
            status_up=requests.post(f"http://127.0.0.1:3000/pm_notify_status_true/{uid}")

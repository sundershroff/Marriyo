...
def signup(request):
    ...
    if request.method == "POST":
        if request.POST['password'] == request.POST['confirm_password']:
                # response = requests.post('http://127.0.0.1:3000/signup/',data=request.POST)
...
def profile(request,id):
    ...
    try:
        ...
        #for work 
        try:
            work_job_title  = jsondec.decode(mydata['work_job_title'])
        except:
            pass
        try:
            work_company_name = jsondec.decode(mydata['work_company_name'])
        except:
            pass
        try:
            work_start_date = jsondec.decode(mydata['work_start_date'])
        except:
            pass
        try:
            starting_salary = jsondec.decode(mydata['starting_salary'])
        except:
            pass
        try:
            work_end_date = jsondec.decode(mydata['work_end_date'])
        except:
            pass
        try:
            final_salary = jsondec.decode(mydata['final_salary'])
        except:
            pass
        try:
            reason_leaving = jsondec.decode(mydata['reason_leaving'])
        except:
            pass
        try:
            work_review_y = jsondec.decode(mydata['work_review_y'])
        except:
            pass
        try:
            expr_certi = jsondec.decode(mydata['expr_certi'])
        except:
            pass
        working_details = []
        if mydata['work_job_title'] != "empty":
...
def hiring_manager_doc(request,id):
    ...
    hiring_my_data = requests.get(f"http://127.0.0.1:3000/hm_my_data/{id}").json()[0] 
    #for education 
    ...
    if request.method == "POST":
        ...
        if request.method == "POST":
            ...
            print("front data",dictio)
            # response = requests.post(f"http://127.0.0.1:3000/profileidcard/{id}",   files=request.FILES)
            response = requests.post(f"http://127.0.0.1:3000/hiring_upload_account/{id}",   data = dictio,files=request.FILES)
            print(response)
            print(response.status_code)
            # print(response.text)
            # uidd = (response.text[1:-1])
...
def oldform(request,id):
    ...
    mydata = requests.get(f"http://127.0.0.1:3000/hm_my_data/{id}").json()[0]  
    hiring_manager(request,id)
    hiring_my_data = requests.get(f"http://127.0.0.1:3000/hm_my_data/DYOX59E9QOT").json()[0]  
    education  = jsondec.decode(hiring_my_data['level_education'])
    study = jsondec.decode(hiring_my_data['field_study'])
    #country api
    neww=[]
